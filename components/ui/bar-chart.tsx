...
export function ChartBarInteractive({ userData }: ChartBarInteractiveProps) {
  // Use user data if provided, otherwise fall back to default chart data
  const hasUserData = userData && userData.length > 0;

  const [activeChart, setActiveChart] =
    React.useState<keyof typeof chartConfig>(hasUserData ? "users" : "desktop")
    ...
    <Card className="py-0">
            ...
            : ["desktop", "mobile"].map((key) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-lg leading-none font-bold sm:text-3xl">
                    {(total[key as keyof typeof total] as number)?.toLocaleString() || "0"}
                  </span>
                </button>
              )
            })}
      ...
      <CardContent className="px-2 sm:p-6">
        ...
        >
          <BarChart
            accessibilityLayer
            data={hasUserData && userData ? userData : chartData}
            margin={{
              left: 12,
              right: 12,
            }}
          >
            ...
            <XAxis
              ...
              interval={0}
              tickFormatter={(value) => {
                // If it's already formatted as month/year string, return as is
            ...
            <ChartTooltip
              content={
                <ChartTooltipContent
                  className="w-[150px]"
                  nameKey={hasUserData ? "users" : "views"}
                  labelFormatter={(value) => {
                    // If it's already a formatted month string, return as is
                    if (typeof value === 'string' && value.includes(' ')) {
                      return value;
                    }
                    // Otherwise format as date
